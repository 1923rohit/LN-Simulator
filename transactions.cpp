...
void process_payments_previous_imple(network* cur_network, const network_params& net_param, vector<transaction>& transactions_to_execute){
...
vector<transaction> get_random_transactions(const network_params& net_params){
    ...
    for(int i=0;i<net_params.num_of_txn;i++){
        ...
        new_txn.amount = rand()%(net_params.txn_fee_upper_limit)+1;
        new_txn.fee=0;
        new_txn.status=0;
...
vector<path_var> get_payment_path(transaction& req, network* cur_network,const network_params& net_params){
